...
pub struct Board {
    ...
    //Castling rights är just nu oanvänd. So sad
    castling_rights : u64,
    turn : bool,
...
impl Board {
    ...
    fn gen_moves(&self, square : u64, index : u64) -> Option<u64> {
        ...
        let moves = match self.square_to_piece(square) {
            ...
            _ => panic!("AAA") //Något har gått väldigt fel
        };
    ...
    fn pawn_moves(&self, index : u64)   -> u64 {
        ...
        if (index < 16) & (!offset) || (index > 47) & offset { //Bonde har inte rört sig
            ...
            if 0 == (both_pices) & (1 << ((index + 8) - 16 * offset as u64)) {
                multiplier |= (1 << ((index + 16) - 32 * offset as u64)) & !both_pices;
            }
        }

        multiplier |= (1 << ((index + 8) - 16 * offset as u64)) & !both_pices;
    ...
    fn push_move(&mut self, from : u64, to : u64, promote : Option<PieceType>) {
        ...
        if piece == PieceType::Pawn && (to >= 1 << 56 || to < 1 << 8) { //Om vi ska promota
            match promote {
                Some(n) => piece = n,
    ...
    fn is_check(&self) -> bool {
        ...
        for i in 0..64 {
            if (0 != self.illegal_moves[i] & king) && (0 != if self.turn {self.white_pieces} else {self.black_pieces} & (1 << i)) {
                //Om något attakerar kungen och om det är en motståndar pjäs så står vi i schack
                return true
            }
        }
        false
...
impl Game {
    ...
    pub fn import_fen(fen : String) -> Self {
